...
class fDepTT{
	...
	*/
	async getDepTT(){
		...
		// RouteSearch
		try{
			aDepTTResult = await this.helper.hClient.departures(this.StationFrom.id.toString(), {"when": new Date(new Date().getTime() + this.DepsOffsetMin * 60000), "results":this.NumDeps,"duration":60,"language":"de","remarks":true});
