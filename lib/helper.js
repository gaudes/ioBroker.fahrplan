...
export default class fHelpers {
	...
	async ReportingError(
		...
		try {
			...
			if (NameSubFunction !== "") {
				sErrMsg = `${sErrMsg}(${NameSubFunction})`;
			}
			if (Err.name !== "HANDLED") {
				sErrMsg = `${sErrMsg} [${Err}]`;
			}
				...
				this.Sentry &&
					this.Sentry.withScope(scope => {
						...
						if (Info.hafas !== null) {
							...
							if (
								Info.hafas.code === "CGI_READ_FAILED" ||
								...
								(Info.hafas.code === "PARSE" &&
									Err.message ===
										"Parser error: root.svcResL.svcResL.res.common.rtSrcL.rtSrcL.type()" &&
									Info.hafas.url === "https://fahrplan.oebb.at/bin/mgate.exe?") ||
	 ...
	 *
	 * @param Level Level for ioBroker Logging
	 * @param Category Category of information
	 * @param Message Message
	 * @param NameClass Name of the class where error occured
	 * @param NameFunction Name of the function where error occured
	 * @param NameSubFunction Name of the subfunction where error occured
	 ...
	 * Helper function for StationSearch in Admin
	 *
	 * @param sProvider Configured provider in Admin
	 * @param sSearchString Searchstring entered in Admin
	 */
	async getStation(sProvider, sSearchString) {
		try {
			this.ReportingInfo(
				"Debug",
				"Adapter",
				`Executing HAFAS search for Stations: Provider = ${sProvider} SearchString = ${sSearchString}`,
				"fHelpers",
				"getStation",
			);
			const sResult = await this.hClient.locations(sSearchString, {
				results: 10,
				stops: true,
				addresses: false,
				poi: false,
			...
			this.ReportingInfo(
				"Debug",
				"Adapter",
				`Result received for search: ${JSON.stringify(sResult)}`,
				"fHelpers",
				"getStation",
				"",
			...
			this.ReportingError(e, `Exception receiving Stations`, "fHelpers", "createHTML", "", {
				provider: sProvider,
	 ...
	 *
	 * @param config Configuration object from Admin
	 */
	async verifyConfig(config) {
		try {
			...
			if (config.routes && Array.isArray(config.routes)) {
				...
				for (const element of config.routes) {
					...
					if (element.enabled === true) {
						if (
							element.station_from === "0" ||
							element.station_from === "" ||
							(await Station.verifyStation(element.station_from)) === false
						) {
							Result.result = false;
							Result.msg.push({
								text: "FahrplanConfigErrorStationFrom",
								arg1: element.station_from,
								arg2: CounterRoutes,
							});
						}
						if (
							element.station_to === "0" ||
							element.station_to === "" ||
							(await Station.verifyStation(element.station_to)) === false
						) {
							Result.result = false;
							Result.msg.push({
								text: "FahrplanConfigErrorStationTo",
								arg1: element.station_to,
								arg2: CounterRoutes,
							});
						}
						if (element.station_via !== "") {
							if (
								element.station_via === "0" ||
								(await Station.verifyStation(element.station_via)) === false
							) {
								Result.result = false;
								Result.msg.push({
									text: "FahrplanConfigErrorStationVia",
									arg1: element.station_via,
									arg2: CounterRoutes,
								});
							}
						}
						if (element.station_from === element.station_to) {
							Result.result = false;
							Result.msg.push({
								text: "FahrplanConfigErrorStationDuplicate",
								arg1: element.station_to,
								arg2: CounterRoutes,
							});
						}
				...
				for (const element of config.departuretimetable) {
					if (element.enabled === true) {
						if (
							...
							Result.result = false;
							Result.msg.push({
								text: "FahrplanConfigErrorStationDepTT",
								arg1: element.station_from,
								arg2: CounterDepTT,
							});
						}
			...
			this.ReportingInfo(
				"Debug",
				"Adapter",
				`Config verified: ${JSON.stringify(Result)}`,
				"fHelpers",
				"verifyConfig",
				"",
	 ...
	 * Sets Text State
	 *
	 * @param sStateName Name of the State
	 * @param sDisplayName Displayed Name of the State
	 * @param sDescription Description of the State
	 * @param sValue Value of the State
	 * @param sRole Role of the State
	 */
	async SetTextState(sStateName, sDisplayName, sDescription, sValue, sRole = "state") {
		try {
			...
			this.ReportingError(e, `Exception writing State`, "fHelpers", "SetTextState", "", {
				name: sStateName,
	...
	async SetNumState(sStateName, sDisplayName, sDescription, sValue, sRole = "value") {
		try {
			...
			this.ReportingError(e, `Exception writing State`, "fHelpers", "SetNumState", "", {
				name: sStateName,
				displayname: sDisplayName,
				description: sDescription,
				value: sValue,
	 ...
	 * @param sDisplayName Displayed Name of the State
	 * @param sDescription Description of the State
	 * @param bValue Value of the State
	 */
	async SetBoolState(sStateName, sDisplayName, sDescription, bValue) {
		try {
			...
			this.ReportingError(e, `Exception writing State`, "fHelpers", "SetBoolState", "", {
				name: sStateName,
				displayname: sDisplayName,
				description: sDescription,
				value: bValue,
	 ...
	 *
	 * @param sStateName Name of the Channel
	 * @param sDisplayName Displayed Name of the Channel
	 * @param sDescription Description of the Channel
	 */
	async SetChannel(sStateName, sDisplayName, sDescription) {
		try {
			...
			this.ReportingError(e, `Exception writing State`, "fHelpers", "SetChannel", "", {
				name: sStateName,
				displayname: sDisplayName,
				description: sDescription,
			});
	...
	async deleteObject(sStateName) {
		try {
			...
			if (CurrentObj) {
				if (this.isUnloaded === false) {
					await this.Fahrplan.delObjectAsync(sStateName);
				}
			} else {
				const CurrentState = await this.Fahrplan.getStateAsync(sStateName);
				if (CurrentState) {
					if (this.isUnloaded === false) {
						await this.Fahrplan.deleteStateAsync(sStateName);
					}
				}
			}
		} catch (e) {
	 ...
	 * Sets boolean State
	 *
	 * @param iRoute Number of Route from configuration
	 */
		...
		try {
			...
			for (const State of States) {
				if (State["_id"] !== `${this.Fahrplan.name}.${this.Fahrplan.instance}.${iRoute}.Enabled`) {
					...
					if (CurrentObj) {
						if (this.isUnloaded === false) {
							await this.Fahrplan.delObjectAsync(State.toString());
						}
					} else {
						const CurrentState = await this.Fahrplan.getStateAsync(State.toString());
						if (CurrentState) {
							if (this.isUnloaded === false) {
								await this.Fahrplan.deleteStateAsync(State.toString());
							}
						}
					}
				}
	 ...
	 * Sets boolean State
	 *
	 * @param iDepTT Number of Departure Timetable from configuration
	 */
	async deleteDepTT(iDepTT) {
		try {
			...
			for (const State of States) {
				if (
					State["_id"] !==
					`${this.Fahrplan.name}.${this.Fahrplan.instance}.DepartureTimetable${iDepTT}.Enabled`
				) {
					// Verify that associated object exists
					const CurrentObj = await this.Fahrplan.getObjectAsync(State.toString());
					if (CurrentObj) {
	 ...
	 *
	 * @param iRoute Number of Route from configuration
	 * @param iConnection Number of Connection
	 * @param iSections Number of Sections in Connection
	 */
	async deleteUnusedSections(iRoute, iConnection, iSections) {
		...
		try {
			...
			this.ReportingInfo(
				"Debug",
				"Route",
				`Delete unused sections for Route #${iRoute}, Connection #${iConnection} with max section # ${iSections} and regex:${SectionRegEx}`,
				"fHelpers",
				"deleteUnusedSections",
			);
			...
			this.ReportingError(
				e,
				`Exception removing unused Sections`,
				"fHelpers",
				"deleteUnusedSections",
				"DeleteStates",
	 ...
	 *
	 * @param iRoute Number of Route from configuration
	 * @param iConnections Number of Connections in Route
	 */
	async deleteUnusedConnections(iRoute, iConnections) {
		...
		try {
			...
			this.ReportingInfo(
				"Debug",
				"Route",
				`Delete unused connections for Route #${iRoute} with max Connection #${iConnections} and regex:${SectionRegEx}`,
				"fHelpers",
			...
			this.ReportingError(
				e,
				`Exception removing unused Connections`,
				"fHelpers",
				"deleteUnusedConnections",
				"DeleteStates",
				{ json: JSON.stringify(Status) },
	 ...
	 *
	 * @param iDepTT Number of Departure Timetable from configuration
	 * @param iDepartures Number of Departures in Departure Timetable
	 */
	async deleteUnusedDepartures(iDepTT, iDepartures) {
		...
		try {
			...
			this.ReportingInfo(
				"Debug",
				"Departure Timetable",
				`Delete unused departures for Departure Timetable #${iDepTT} with max Departures #${iDepartures} and regex:${SectionRegEx.toString()}`,
				"fHelpers",
				"deleteUnusedDepartures",
			);
			for (const State in States) {
				const Searcher = State.toString().match(new RegExp(SectionRegEx));
				if (Searcher !== null) {
			...
			this.ReportingError(
				e,
				`Exception removing unused Departures`,
				"fHelpers",
				"deleteUnusedDepartures",
				"DeleteStates",
				{ json: StatesJSON },
