	...
	async verifyConfig(config){
		try{
			...
			if (config.routes && Array.isArray(config.routes)){
				let CounterRoutes = 0;
				await config.routes.forEach(async element =>{
					if (element.enabled === true){
						if (element.station_from === "0" || await Station.verifyStation(element.station_from) === false){
							Result.result = false;
							Result.msg.push(`Unknown Station-From in Route #${CounterRoutes}`);
						}
						if (element.station_to === "0" || await Station.verifyStation(element.station_to) === false){
							Result.result = false;
							Result.msg.push(`Unknown Station-To in Route #${CounterRoutes}`);
						}
						if (element.station_via !== ""){
							if (element.station_via === "0" || await Station.verifyStation(element.station_via) === false){
								Result.result = false;
								Result.msg.push(`Unknown Station-Via in Route #${CounterRoutes}`);
							}
						}
					}
					CounterRoutes++;
			...
			if (config.departuretimetable && Array.isArray(config.departuretimetable)){
				let CounterDepTT = 0;
				await config.departuretimetable.forEach(async element =>{
					if (element.enabled === true){
						if (await Station.verifyStation(element.station_from) === false){
							Result.result = false;
							Result.msg.push(`Unknown Station-From in Departure Timetable #${CounterDepTT}`);
						}
					}
					CounterDepTT++;
				});
