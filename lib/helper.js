	...
	*/
	async deleteUnusedConnections(iRoute, iConnections){
		...
		try{
			...
			Channels = await this.Fahrplan.getChannelsAsync();
			if (Channels){
				for (const Channel of Channels){
					if (Channel["_id"]){
						const Searcher = Channel["_id"].toString().match(new RegExp(SectionRegEx));
			...
			this.ErrorReporting(e, `Exception removing unused Connections`, "fHelpers", "deleteUnusedConnections", "",{json: JSON.stringify(Channels)});
		}
	}
	//#endregion
