	...
	*/
	async checkDelay(BasePath, RouteIndex){
		try {
			...
			if (RouteDelays.length > 0) this.helper.ReportingInfo("Debug", "Route", `Configured Delays for Route #${RouteIndex}: ${JSON.stringify(RouteDelays)}`, "fJourney", "checkDelay", "", JSON.stringify(RouteDelays));
				...
				if ((RouteDelay.departplan === "" || RouteDelay.departplan === this.helper.Fahrplan.formatDate(new Date(this.departure), "hh:mm")) && ( RouteDelay.days.includes("7") || RouteDelay.days.includes(new Date(this.departure).getDay().toString()))){
					...
					if (new Date() >= new Date(NotiStartTime) && new Date() <= new Date(this.departure) ){
						...
						if (this.departureDelaySeconds !== 0 && this.departureDelaySeconds >= (this.helper.Fahrplan.config.DelayTime * 60)){
							...
							if (OldNotifyValue === null || !OldNotifyValue || (OldNotifyValue && OldNotifyValue.val !== null && parseInt(this.departureDelaySeconds.toString()) !== parseInt(OldNotifyValue.val.toString()))){
								this.notify = true;
	...
	*/
	async buildDelayNotification(){
		try{
			...
			this.helper.ReportingError(e, `Exception in Journey`, "fJourney", "buildDelayNotification", "", {json: this.json});
			throw this.helper.ErrorCustom("HANDLED");
		}
	}
	...
	*/
	async writeJourneyHTML(BasePath){
		try {
			...
			this.helper.ReportingError(e, `Exception in Journey`, "fJourney", "writeJourneyHTML", "", {json: this.json});
			throw this.helper.ErrorCustom("HANDLED");
		}
	}
}
