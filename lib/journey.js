...
class fJourney{
	...
	parseJourney(Journey){
		...
		try{
			...
			for (let iSectionCurrent in Journey.legs ) {
				...
				let CurrSection = new fSection(this.helper);
			...
			throw new Error(`Exception in fJourney/parseJourney [${e}]`);
		} 
	}
	...
	*/
	async checkDelay(BasePath, RouteIndex){
		try {  
			let ConfigDelays = this.helper.Fahrplan.config.delays||[];
				...
				if ((RouteDelay.departplan === "" || RouteDelay.departplan === this.helper.Fahrplan.formatDate(new Date(this.departure), "hh:mm")) && ( RouteDelay.days.includes("7") || RouteDelay.days.includes(new Date(this.departure).getDay().toString()))){
					...
					if (new Date() >= new Date(NotiStartTime) && new Date() <= new Date(this.departure) ){
						...
						if (this.departureDelaySeconds !== 0 && this.departureDelaySeconds >= (this.helper.Fahrplan.config.DelayTime * 60)){
							let OldNotifyValue = await this.helper.Fahrplan.getStateAsync(`${BasePath}.NotifyValue`);
			...
			throw new Error(`Exception in fJourney/checkDelay [${e}]`);
		} 	
	}
	...
	*/
	async buildDelayNotification(){
		try{
			...
			throw new Error(`Exception in fJourney/buildDelayNotification [${e}]`);
		} 
	}	
	...
	*/
	async writeJourney(BasePath, RouteIndex, JourneyIndex){
		try {  
			...
			throw new Error(`Exception in fJourney/writeJourney [${e}]`);
		} 	
	}
	...
	*/
	createHTML(){
		try{
			...
			throw new Error(`Exception in fJourney/createHTML [${e}]`);
		} 
	} 
	...
	*/
	async writeJourneyHTML(BasePath){
		try {
			...
			throw new Error(`Exception in fJourney/writeJourneyHTML [${e}]`);
		} 
	}	
}
