	...
	*/
	async setLine(Section){
		try {
			...
			this.helper.ErrorReporting(e, `Exception in Line`, "fLine", "setLine", {json: Section});
	...
	*/
	async writeLine(BasePath){
		try {
			...
			this.helper.ErrorReporting(e, `Exception in Line`, "fLine", "writeLine", {json: this.json});
			throw this.helper.ErrorCustom("HANDLED");
		}
	}
	...
	*/
	translateProduct(){
		try {
			...
			this.helper.ErrorReporting(e, `Exception in Line`, "fLine", "translateProduct");
			throw this.helper.ErrorCustom("HANDLED");
		}
	}
}
