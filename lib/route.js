...
const fRouteOptions = require("./routeoptions.js");
	...
	*/
	async writeBaseStates(){
		try {
			...
			this.helper.ErrorReporting(e, `Exception in Route`, "fRoute", "writeBaseStates");
	...
	*/
	async writeStates(){
		try {
			...
			this.helper.ErrorReporting(e, `Exception in Route`, "fRoute", "writeStates", "", {json: this.json} );
			throw this.helper.ErrorCustom("HANDLED");
		}
	}
	...
	*/
	async writeHTML(){
		try {
			...
			this.helper.ErrorReporting(e, `Exception in Route`, "fRoute", "writeHTML", "", {json: this.json} );
			throw this.helper.ErrorCustom("HANDLED");
		}
	}
	...
	async getRoute(RouteOptions){
		...
		try{
			...
			if (e.hafasErrorCode === "H890"){
				...
				this.JourneysFound = false;
			} else if (e.code === "EAI_AGAIN" || e.name === "FetchError" || e.code === "CGI_READ_FAILED" || e.message === "HCI Service: request failed" || e.message === "Gateway Timeout" ){
				...
				this.helper.ErrorReporting(e, `Exception in Route`, "fRoute", "getRoute", "RouteSearch");
				throw this.helper.ErrorCustom("HANDLED");
			}
		}
		...
		try{
			...
			this.helper.ErrorReporting(e, `Exception in Route`, "fRoute", "getRoute", "Iteration", {json: this.json});
			throw this.helper.ErrorCustom("HANDLED");
		}
	}
}
