export default class fStation {
	 ...
	 *
	 * @param eBhf Numeric ID for station
	 */
	async getStation(eBhf) {
		...
		try {
			...
			if (e.hafasErrorCode === "H890") {
				...
				this.helper.ReportingError(e, `HAFAS error in Station`, "fStation", "getStation", "", {
					json: eBhf,
					hafas: {
						request: e.request,
						url: e.url,
	...
	async verifyStation(eBhf) {
		...
		try {
			...
			if (e.isHafasError === true && e.message !== "location/stop not found") {
				this.helper.ReportingError(e, `HAFAS error in Station`, "fStation", "verifyStation", "", {
					json: eBhf,
					hafas: {
						request: e.request,
						url: e.url,
	...
	async writeStation(BasePath, BaseDesc) {
		try {
			...
			if (this.platformplanned !== null) {
				await this.helper.SetTextState(
					`${BasePath}.PlatformPlanned`,
					`${BaseDesc} PlatformPlanned`,
					`${BaseDesc} PlatformPlanned`,
					this.platformplanned,
				);
			}
